...
export function OrderCard({ order, onAction }: Props) {
  ...
  return (
    <View className="bzp-order-card" onClick={() => Taro.navigateTo({ url: `/pages/order/detail/index?id=${order.id}` })}>
      <View className="bzp-order-card__head">
        <Text>{order.orderNo}</Text>
        <Text className="bzp-order-card__status">{orderStatusText[order.status] || order.status}</Text>
      </View>
      <Text className="bzp-order-card__slot">
      ...
      {uniqueBatchTags.length ? (
        <View className="bzp-order-card__batch">
          {uniqueBatchTags.map((tag) => <Text key={tag}>{tag}</Text>)}
        </View>
      ) : null}
      <Text className="bzp-order-card__items">共 {order.items.reduce((total, item) => total + item.quantity, 0)} 件商品</Text>
      <View className="bzp-order-card__foot">
        <PriceText value={order.payableAmount} size="small" />
        {actions.length ? (
          <View className="bzp-order-card__actions">
            {actions.map((action) => (
              <Button
                key={action.id}
                className={action.primary ? 'bzp-order-card__primary' : 'bzp-order-card__ghost'}
                onClick={(event) => {
